...
def _extract_close_series(df: pd.DataFrame, ticker: str):
    if df is None or df.empty:
        return None
    if isinstance(df.columns, pd.MultiIndex):
        for col_name in [('Adj Close', ticker), ('Close', ticker), ('Adj Close', ''), ('Close', '')]:
...
def download_yahoo_series(ticker: str, series_name: str, start_date: str) -> pd.DataFrame:
    """Robust Yahoo download with fallback methods for Streamlit Cloud."""
    attempts = []
    ...
    try:
        ...
        attempts.append(('download_start', df))
    except Exception:
        attempts.append(('download_start', pd.DataFrame()))

    # Attempt 2: long period fallback
    try:
        if attempts[-1][1] is None or attempts[-1][1].empty:
            df2 = yf.download(
                ticker,
                period='max',
                progress=False,
            ...
            attempts.append(('download_max', df2))
    except Exception:
        attempts.append(('download_max', pd.DataFrame()))

    # Attempt 3: Ticker.history fallback
    try:
        if attempts[-1][1] is None or attempts[-1][1].empty:
            df3 = yf.Ticker(ticker).history(start=start_date, auto_adjust=False)
            attempts.append(('ticker_history', df3))
    except Exception:
        attempts.append(('ticker_history', pd.DataFrame()))
    ...
    if isinstance(chosen.index, pd.DatetimeIndex):
        chosen = chosen.reset_index()

    s = _extract_close_series(chosen, ticker)
    if s is None:
        return pd.DataFrame(columns=['date', series_name])

    out = pd.DataFrame({'date': pd.to_datetime(chosen.iloc[:, 0]), series_name: pd.to_numeric(s, errors='coerce')})
    out['date'] = pd.to_datetime(out['date']).dt.tz_localize(None)
    out = out.dropna(subset=[series_name]).sort_values('date').drop_duplicates(subset=['date']).reset_index(drop=True)

    # respect requested start date if period=max was used
    out = out[out['date'] >= pd.to_datetime(start_date)].reset_index(drop=True)
    return out
...
def fetch_market_data(start_date: str) -> pd.DataFrame:
    ...
    required = ['gold_usd_oz', 'usd_inr']
    missing_required = [name for name in required if market_frames[name].empty]
    if missing_required:
        raise ValueError(
            'Could not download required Yahoo Finance series: ' + ', '.join(missing_required) +
            '. This is usually a temporary Yahoo/Streamlit Cloud issue. Re-run the app once, or use the fixed download logic.'
        )
    ...
    essential_cols = ['gold_usd_oz', 'usd_inr', 'crude_oil', 'silver_usd_oz', 'sp500', 'gold_inr_10g']
    df = df.dropna(subset=essential_cols).reset_index(drop=True)
    if 'nifty50' in df.columns:
        df['nifty50'] = df['nifty50'].ffill().bfill()

    if df.empty:
        raise ValueError('Data download succeeded but cleaning removed all rows. Try an earlier start date like 2010-01-01.')
    return df
...
def build_features(clean_df: pd.DataFrame):
    ...
    if model_df.empty:
        raise ValueError('No rows left after feature engineering. Use an earlier start date.')
...
def train_pipeline(start_date: str, train_ratio: float, val_ratio: float):
    ...
    if train_end <= 0 or val_end <= train_end or val_end >= n:
        raise ValueError('Invalid split sizes. Adjust train and validation ratios.')
...
st.caption('Streamlit version using Yahoo Finance market data and leakage-safe feature engineering.')
...
if run_btn:
    try:
        ...
        st.error(str(e))
        st.info('Try clicking Run pipeline again. If the issue remains, replace your app file with the fixed version below.')
else:
